...
// 将文件信息存储到数据库
  MediaFiles mediaFiles = addMediaFilesToDb(companyId, fileMd5, uploadFileParamsDto,
...
// 准备返回数据
 ...
 public RestResponse mergechunks(long companyId, String fileMd5, int chunkTotal, UploadFileParamsDto uploadFileParamsDto) {
  //=====获取分块文件路径=====
  String chunkFileFolderPath = getChunkFileFolderPath(fileMd5);
  // 组成最终分块文件路径组成 List<ComposeSource>
...
//=====清除分块文件=====
 ...
 /**
  * 清除分块文件
  * @param chunkFilePathPath 分块文件路径
  * @param chunkTotal 分块文件总数
  */
 private void clearChunkFiles(String chunkFilePathPath, int chunkTotal) {

  try {
   List<DeleteObject> deleteObjects = Stream.iterate(0, i -> ++i)
           .limit(chunkTotal)
           .map(i -> new DeleteObject(chunkFilePathPath.concat(Integer.toString(i))))
           .collect(Collectors.toList());
   ...
   results.forEach(r -> {
    ...
    try {
     ...
     e.printStackTrace();
     log.error("清差分块文件失败,objectname:{}", deleteError.objectName(), e);
    }
   });

  } catch (Exception e) {
   e.printStackTrace();
   log.error("清差分块文件失败,chunkFilePathPath:{}", chunkFilePathPath, e);
  }
